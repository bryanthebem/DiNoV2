...
class NotionIntegration:
    ...
    def _parse_summary_to_notion_blocks(self, summary_text: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            # Tenta identificar cabeçalhos em negrito como **Problema:**
            # O pattern '^\*\*(.*?):\*\*$' captura texto entre **:** no início da linha
            bold_heading_match = re.match(r'^\*\*(.*?):\*\*$', line)
            if bold_heading_match:
                # Se for um título em negrito, cria um bloco de parágrafo com texto negrito
                notion_blocks.append({
                    "object": "block",
                    "type": "paragraph",
                    "paragraph": {
                        "rich_text": [{"type": "text", "text": {"content": bold_heading_match.group(1) + ":", "annotations": {"bold": True}}}]
                    }
                })
            # Tenta identificar itens de lista como * Item ou - Item
            elif line.startswith('* ') or line.startswith('- '):
                # Remove o prefixo da lista e processa o restante para negrito/itálico
                content_text = line[2:]
                notion_blocks.append({
                    "object": "block",
                ...
                # Para qualquer outro texto, trata como um parágrafo normal
                notion_blocks.append({
                    "object": "block",
                    "type": "paragraph",
                    "paragraph": {
